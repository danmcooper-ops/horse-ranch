...
public final class WorldBuilder {
    ...
    public static RanchWorld build() {
        ...
        MeshPartBuilder grass = mb.part("grass", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.42f, 0.66f, 0.3f, 1f)));
        BoxShapeBuilder.build(grass, 0f, -0.05f, 0f, 2f * HALF + 30f, 0.1f, 2f * HALF + 30f);

        MeshPartBuilder patches = mb.part("patches", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.37f, 0.6f, 0.26f, 1f)));
        for (int i = 0; i < 40; i++) {
        ...
        MeshPartBuilder dirt = mb.part("trail", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.62f, 0.5f, 0.34f, 1f)));
        BoxShapeBuilder.build(dirt, 0f, 0.008f, -20f, 3.4f, 0.012f, 60f);
        BoxShapeBuilder.build(dirt, -16f, 0.008f, -20f, 32f, 0.012f, 3.4f);

        // --- Perimeter fence ----------------------------------------------
        Material wood = mat(new Color(0.55f, 0.4f, 0.24f, 1f));
        MeshPartBuilder fence = mb.part("fence", GL20.GL_TRIANGLES, ATTRS, wood);
        for (int side = 0; side < 4; side++) {
            boolean horizontal = side < 2;              // along X at z = +/-HALF
            float fixed = side % 2 == 0 ? HALF : -HALF;
            for (float p = -HALF; p <= HALF; p += 5.5f) {
                if (horizontal) BoxShapeBuilder.build(fence, p, 0.65f, fixed, 0.22f, 1.3f, 0.22f);
                else BoxShapeBuilder.build(fence, fixed, 0.65f, p, 0.22f, 1.3f, 0.22f);
            }
            for (int rail = 0; rail < 2; rail++) {
                float y = 0.55f + rail * 0.5f;
                if (horizontal) BoxShapeBuilder.build(fence, 0f, y, fixed, 2f * HALF, 0.12f, 0.1f);
                else BoxShapeBuilder.build(fence, fixed, y, 0f, 0.1f, 0.12f, 2f * HALF);
            }
        }
        ...
        // --- Barn ----------------------------------------------------------
        float bx = -32f, bz = -20f;
        MeshPartBuilder barn = mb.part("barn", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.72f, 0.22f, 0.18f, 1f)));
        BoxShapeBuilder.build(barn, bx, 2.5f, bz, 10f, 5f, 14f);
        MeshPartBuilder roof = mb.part("roof", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.42f, 0.3f, 0.24f, 1f)));
        ...
        MeshPartBuilder door = mb.part("barnDoor", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.35f, 0.2f, 0.14f, 1f)));
        ...
        MeshPartBuilder hay = mb.part("hay", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.85f, 0.72f, 0.35f, 1f)));
        ...
        MeshPartBuilder water = mb.part("water", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.3f, 0.55f, 0.8f, 1f)));
        BoxShapeBuilder.build(water, -24f, 0.62f, -26f, 2.3f, 0.1f, 0.8f);
        world.addObstacle(-24f, -26f, 3f, 1.5f);

        // --- Trees ---------------------------------------------------------
        MeshPartBuilder trunks = mb.part("trunks", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.45f, 0.32f, 0.2f, 1f)));
        MeshPartBuilder leaves = mb.part("leaves", GL20.GL_TRIANGLES, ATTRS,
                mat(new Color(0.2f, 0.5f, 0.22f, 1f)));
        int planted = 0;
        while (planted < 26) {
            ...
            CylinderShapeBuilder.build(trunks, 0.5f, h, 0.5f, 7);
            leaves.setVertexTransform(tr.setToTranslation(x, h + 1.2f, z));
            ConeShapeBuilder.build(leaves, 2.8f, 2.6f, 2.8f, 8);
            leaves.setVertexTransform(tr.setToTranslation(x, h + 2.9f, z));
            ConeShapeBuilder.build(leaves, 2f, 2f, 2f, 8);
